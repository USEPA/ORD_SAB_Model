...
test <- vroom("/work/GRDVULN/PWS/data/RF_Testing/testing.csv", col_types = c("Correct"="f"))
# 
# true.train <- train%>%
#   filter(Correct == TRUE)
...
for(n in 1:nrow(perf)){
  ...
  auc <- roc_auc(probs,Correct,.pred_TRUE)
  
  # Split into classes by service connections
  auc.cut <- data.frame(LT_SC = max(probs$Service_Connections_Count),
                        AUC = auc)
  
  for(n in c(500,1000,5000,25000,50000,100000)){
    probs.filt <- probs%>%
      filter(Service_Connections_Count < n)
    
    filt.auc <- roc_auc(probs.filt,Correct,.pred_TRUE)
    
    newRow <- data.frame(LT_SC = n,
                         AUC = filt.auc)
    auc.cut <- rbind(auc.cut,newRow)
  }
