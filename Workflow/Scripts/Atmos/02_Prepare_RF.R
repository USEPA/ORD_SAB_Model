...
df.dist <- vroom(paste0("/work/GRDVULN/PWS/outputs/dist_tables/",filename,"_pws_dist_all.csv"),col_types = c("GISJOIN"="c", "PWSID"="c",
...
vroom_write(join.all, paste0("/work/GRDVULN/PWS/data/RF_Tables/",filename,".csv"),delim = ",", append = FALSE)
...
if(state.sel %in% c("CA","AR","AZ","NJ","CT","TX")){
  ...
  # Save file
  vroom_write(filt, paste0("/work/GRDVULN/PWS/data/RF_Valid/",filename,".csv"))
}
