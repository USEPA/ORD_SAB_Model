...
# List census block file paths
# block.file <- data.frame(file = list.files("/work/GRDVULN/census", pattern = "2020.shp$"),
#                          path = list.files("/work/GRDVULN/census", pattern = ".shp$", full.names = TRUE))%>%
...
# For every block centroid, find the distance of every SDWIS location within 50 miles
...
print(paste0("Cluster created with ",cores," cores ... Beginning to Build Dataset of ",nrow(blks.sf)," Blocks ... ",Sys.time()))
...
vroom_write(df.new,paste0("/work/GRDVULN/PWS/outputs/dist_tables/",filename,"_pws_dist_all.csv"), delim = ",", append = FALSE)
end <- Sys.time()
